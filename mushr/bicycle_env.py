...
class BicycleReachEnv(gym.Env):
    ...
    def get_space_limits(self):
        ...
        left_rot_vec = utils.rot_vector(-self.goal_angle_limit, unit_vec)
        right_rot_vec = utils.rot_vector(self.goal_angle_limit, unit_vec)
    ...
    def reset(self, goal=None):
        ...
        if goal is None:
            ...
            self.goal = np.concatenate([utils.rot_vector(ang, np.array([mag, 0])), [0]])
        else:
            self.goal = goal
    ...
    def _terminal(self, s, g):
        return utils.goal_distance(s[:2], g[:2]) < self.distance_threshold
    ...
    def compute_reward(self, ag, dg, info):
        ...
        rewards[np.logical_and(
            ...
            utils.goal_distance(ag[:, :2], dg[:, :2]) < self.distance_threshold
        )] = 0
        # breakpoint()
